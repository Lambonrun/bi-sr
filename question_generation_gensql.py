...
# 并发数控制
CONCURRENT_REQUESTS = 20  # 可以修改这个值来控制并发数
...
async def generate_question_async(session, question: str):
    ...
    """
    prompt = f"""
    Your job is to generate the tables needed to answer the question. You need to write the SQL query to answer the question. You can use the table names in the question to generate the SQL query and create the table names if needed.
    ...
    try:
        tables = Parser(answer_content).tables
        columns = Parser(answer_content).columns
    except:
        print(f"Warning: Could not parse the answer: {answer_content}")
        tables = []
        columns = []
...
async def load_queries_async(file_path: str):
    ...
    try:
        ...
        # 创建aiohttp会话
        connector = aiohttp.TCPConnector(limit=20)  # 限制连接数
...
if __name__ == "__main__":
    ...
    with open("/root/autodl-tmp/schlink/BIRD_Data/data/dev_20240627/dev_question_generation_gensql.json", "w", encoding="utf-8") as f:
