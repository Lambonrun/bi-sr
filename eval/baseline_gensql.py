...
def parse_prediction(pred_str: str):
    ...
    match = re.search(r"database:\s*(.*?), table:\s*(.*)", pred_str, re.IGNORECASE)
    if match:
...
if __name__ == "__main__":
    corpus = load_corpus("/root/autodl-tmp/schlink/BIRD_Data/data/merged_table_baseline.json")
    questions, answers = load_queries("/root/autodl-tmp/schlink/BIRD_Data/data/dev_20240627/cache/dev_question_generation_gensql.json")
    ...
    model = SentenceTransformer("/root/autodl-tmp/model_file/Qwen3-Embedding-4B", device=device)
    ...
    if len(predicted) != len(answers):
        ...
        with open("/root/autodl-tmp/schlink/BIRD_Data/data/dev_20240627/cache/dev_predicted_tables_baseline_gensql.json", "w", encoding="utf-8") as f:
            json.dump({"questions": questions, "predicted_tables": predicted, "answers": answers}, f, ensure_ascii=False, indent=4)
            print("数据保存完成！保存到 /root/autodl-tmp/schlink/BIRD_Data/data/dev_20240627/cache/dev_predicted_tables_baseline_gensql.json")
